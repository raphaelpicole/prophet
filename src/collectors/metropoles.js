 ...
 */
export function parseMetropolesHomepage(html) {
    const articles = [];
    ...
    const cardMatches = html.matchAll(/<(article|div)[^>]*class="[^"]*(?:post|card|news)[^"]*"[^>]*>.*?<h[23][^>]*>.*?<a[^>]*href="([^"]+)"[^>]*>([^<]+)<\/a>.*?<\/h[23]>.*?<\/\1>/gis);
 ...
 */
export function parseMetropolesArticle(html) {
    // Seletores esperados:
