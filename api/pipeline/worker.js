...
export async function runPipeline() {
    ...
    for (const source of sources) {
        const detail = { source: source.name, collected: 0, inserted: 0, errors: [] };
        try {
            ...
            for (const article of relevantArticles) {
                const check = checkDuplicate({ title: article.title, url: article.url }, existingMap);
 ...
 */
async function groupArticlesIntoStories() {
