 ...
 */
export async function analyzeWithBestAvailable(article, options) {
    ...
    async function tryAnalyzer(name, fn) {
        try {
    ...
    const order = preferProvider === 'ollama-cloud'
        ? ['ollama-cloud', 'groq']
        : ['groq', 'ollama-cloud'];
    for (const provider of order) {
        if (provider === 'ollama-cloud' && ollamaKey) {
            const { analyzeWithOllamaCloud } = await import('./ollama-cloud-analyzer.js');
            const result = await tryAnalyzer('ollama-cloud', () => analyzeWithOllamaCloud(article, { apiKey: ollamaKey }));
            if (result)
                return result;
        }
        if (provider === 'groq' && groqKey) {
            const { analyzeWithGroq } = await import('./groq-analyzer.js');
            const result = await tryAnalyzer('groq', () => analyzeWithGroq(article, { apiKey: groqKey }));
            if (result)
                return result;
        }
    }
 ...
 */
export async function analyzeBatchBestAvailable(articles, concurrency = 3) {
    const results = [];
    const errors = [];
    // Processa em batches para não sobrecarregar
    for (let i = 0; i < articles.length; i += concurrency) {
        const batch = articles.slice(i, i + concurrency);
        const batchResults = await Promise.allSettled(batch.map(article => analyzeWithBestAvailable(article)));
