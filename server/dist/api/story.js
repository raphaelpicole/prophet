...
export default withSentry(async function handler(req, res) {
    ...
    const { data: analysis } = await supabase
        ...
        .in('article_id', (articles || []).map((a) => a.article?.id).filter(Boolean));
    // Calcula média de viés e sentimento
    const avgBias = analysis?.length
        ? analysis.reduce((s, a) => s + (a.bias_score || 0), 0) / analysis.length
        : null;
    const avgSentiment = analysis?.length
        ? analysis.reduce((s, a) => s + (a.sentiment_score || 0), 0) / analysis.length
        : null;
