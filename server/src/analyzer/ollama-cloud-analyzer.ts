 ...
 */
export async function analyzeWithOllamaCloud(
  article: ArticleToAnalyze,
  config?: Partial<OllamaCloudConfig>
): Promise<AnalysisResult & { used_ollama_cloud: boolean; error?: string; model_used?: string }> {
 ...
 */
export async function analyzeWithOllamaCloudWithRetry(
  article: ArticleToAnalyze,
  config?: Partial<OllamaCloudConfig>,
  maxRetries: number = 2
): Promise<AnalysisResult & { used_ollama_cloud: boolean; error?: string; model_used?: string }> {
...
function parseOllamaCloudResponse(raw: string): AnalysisResult {
  try {
...
export async function testOllamaCloud() {
  ...
  const testArticles: ArticleToAnalyze[] = [
    {
      id: 'test-1',
