...
import type { Analysis, Entity } from '../types.js';
...
export async function analyzeArticle(articleId: string, title: string, content?: string): Promise<Analysis> {
  ...
  if (error) console.error('Erro salvando análise:', error);
...
async function upsertEntities(articleId: string, entities: Entity[]): Promise<void> {
...
async function linkRegions(articleId: string, regionNames: string[]): Promise<void> {
 ...
 */
export async function analyzePending(limit = 20): Promise<number> {
