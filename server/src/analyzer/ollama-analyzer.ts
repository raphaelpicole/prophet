 ...
 */
export async function isOllamaAvailable(config?: Partial<OllamaConfig>): Promise<boolean> {
 ...
 */
export async function listOllamaModels(config?: Partial<OllamaConfig>): Promise<string[]> {
  const merged = { ...DEFAULT_CONFIG, ...config };
...
): Promise<AnalysisResult & { used_ollama: boolean; error?: string }> {
  const merged = { ...DEFAULT_CONFIG, ...config };
 ...
 */
function parseOllamaResponse(raw: string): AnalysisResult {
  try {
 ...
 */
export async function analyzeBatchWithOllama(
  articles: ArticleToAnalyze[],
  config?: Partial<OllamaConfig>,
  concurrency: number = 2 // Ollama local funciona melhor com menos concorrência
): Promise<{ results: (AnalysisResult & { article_id: string; used_ollama: boolean })[]; errors: { article_id: string; error: string }[] }> {
  const results: (AnalysisResult & { article_id: string; used_ollama: boolean })[] = [];
