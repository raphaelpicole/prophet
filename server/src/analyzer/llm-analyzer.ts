 ...
 */
function parseLLMResponse(raw: string): Partial<AnalysisResult> {
  try {
...
): Promise<AnalysisResult & { used_llm: boolean; error?: string }> {
 ...
 */
export async function analyzeBatchWithLLM(
  articles: ArticleToAnalyze[],
  config?: Partial<LLMConfig>,
  concurrency: number = 3
): Promise<{ results: (AnalysisResult & { article_id: string; used_llm: boolean })[]; errors: { article_id: string; error: string }[] }> {
  const results: (AnalysisResult & { article_id: string; used_llm: boolean })[] = [];
